...
export function formatVersionDiff(current: string, latest: string): { current: string; latest: string; type: string } {
  try {
    const cleanCurrent = semver.coerce(current)?.version || current;
    const cleanLatest = semver.coerce(latest)?.version || latest;
...
export function findAllPackageJsonFiles(rootDir: string = process.cwd(), excludePatterns: string[] = []): string[] {
  ...
  function traverseDirectory(dir: string): void {
    try {
      ...
      for (const file of files) {
        ...
        const skipDirs = ['node_modules', '.git', 'dist', 'build', '.next', 'coverage', '.cache', 'out', '.output', '.nuxt', '.vercel', '.netlify', 'lib', 'es', 'esm', 'cjs'];
...
export function collectAllDependencies(packageJsonFiles: string[]): Array<{ name: string; version: string; type: string; packageJsonPath: string }> {
...
export async function getAllPackageData(packageNames: string[]): Promise<Map<string, { latestVersion: string; allVersions: string[] }>> {
...
export function getOptimizedRangeVersion(packageName: string, currentRange: string, allVersions: string[], latestVersion: string): string {
  try {
...
export function findClosestMinorVersion(installedVersion: string, allVersions: string[]): string | null {
  try {
