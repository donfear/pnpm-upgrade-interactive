...
import { formatVersionDiff } from './utils';
...
export class InteractiveUI {
  ...
  private formatVersionDiff(current: string, target: string, colorFn: (text: string) => string): string {
    if (current === target) {
  ...
  public async selectPackagesToUpgrade(packages: PackageInfo[], previousSelections?: Map<string, 'none' | 'range' | 'latest'>): Promise<PackageUpgradeChoice[]> {
    const outdatedPackages = packages.filter(p => p.isOutdated);
    ...
    const deduplicatedPackages = Array.from(uniquePackages.values()).map(({ pkg, packageJsonPaths }) => ({
      ...pkg,
    ...
    selectedStates
      ...
      .forEach(state => {
        ...
        const targetVersionWithPrefix = this.applyVersionPrefix(state.currentVersionSpecifier, targetVersion);
  ...
  private async interactiveTableSelector(selectionStates: PackageSelectionState[]): Promise<PackageSelectionState[]> {
    return new Promise((resolve) => {
      ...
      const renderPackageLine = (state: PackageSelectionState, index: number): string => {
        ...
        if (state.hasRangeUpdate) {
          ...
          const rangeVersionWithPrefix = this.applyVersionPrefix(state.currentVersionSpecifier, state.rangeVersion);
        ...
        if (state.hasMajorUpdate) {
          ...
          const latestVersionWithPrefix = this.applyVersionPrefix(state.currentVersionSpecifier, state.latestVersion);
      ...
      const ensureVisible = (rowIndex: number) => {
        ...
        scrollOffset = Math.max(0, Math.min(scrollOffset, Math.max(0, states.length - maxVisibleItems)));
      };
      ...
      const renderInterface = () => {
        if (isInitialRender) {
          ...
          console.log('');
          console.log('  ' + chalk.bold.white('↑/↓ ') + chalk.gray('Move') + '  ' + chalk.bold.white('←/→ ') + chalk.gray('Select versions') + '  ' + chalk.bold.white('M ') + chalk.gray('Select all minor') + '  ' + chalk.bold.white('L ') + chalk.gray('Select all updates') + '  ' + chalk.bold.white('U ') + chalk.gray('Unselect all'));
          ...
          const statusLine = totalItems > maxVisibleItems
            ? chalk.gray(`Showing ${chalk.gray(startItem)}-${chalk.gray(endItem)} of ${chalk.gray(totalItems)} packages`) + '  ' + chalk.gray('Enter ') + chalk.gray('Confirm') + '  ' + chalk.gray('Esc ') + chalk.gray('Cancel')
            : chalk.gray(`Showing all ${chalk.gray(totalItems)} packages`) + '  ' + chalk.gray('Enter ') + chalk.gray('Confirm') + '  ' + chalk.gray('Esc ') + chalk.gray('Cancel');
        ...
        switch (key.name) {
          ...
          case 'return':
            ...
            if (selectedCount === 0) {
              ...
              console.log('\n' + chalk.yellow('⚠️  No packages selected. Press ↑/↓ to navigate and ←/→ to select versions, or ESC to exit.'));
  ...
  public async confirmUpgrade(choices: PackageUpgradeChoice[]): Promise<boolean | null> {
    ...
    packagesByName.forEach((packageChoices, packageName) => {
      ...
      const instancesText = packageChoices.length > 1 ? chalk.gray(` (${packageChoices.length} instances)`) : '';
      console.log(`  • ${chalk.cyan(packageName)} → ${upgradeTypeColor(choice.targetVersion)} ${chalk.gray(`(${choice.upgradeType})`)}${instancesText}`);
    });
    ...
    return new Promise((resolve) => {
      ...
      try {
        ...
        ]).then(answer => resolve(answer.proceed)).catch(() => resolve(false));
      }
    });
  }
